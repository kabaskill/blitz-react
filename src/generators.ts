...
// Use a more compatible way to resolve paths
...
async function generatePackageJson(targetDir: string, options: UserOptions): Promise<void> {
  ...
  try {
    ...
    // Make sure to actually write the file
    await fs.writeFile(
      path.join(targetDir, 'package.json'), 
      packageJson
    );
  } catch (error) {
    console.error(chalk.red('Error generating package.json:'), error);
    throw error;
  }
}

export async function generateProjectFiles(targetDir: string, options: UserOptions): Promise<void> {
  const templateBaseDir = path.join(TEMPLATE_DIR, options.template === 'react-ts' ? 'react-ts' : 'react-js');
  const commonTemplatesDir = path.join(TEMPLATE_DIR, 'common');
  
  // Get latest dependency versions for templates
  ...
  try {
    ...
    // Then process template-specific files (which may override common ones)
    await processTemplateDirectory(templateBaseDir, targetDir, options, latestVersions);
  } catch (error) {
    console.error(chalk.red('Error generating project files:'), error);
    throw error;
  }
}
...
async function processTemplateDirectory(
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      // Replace special placeholders in filenames
    ...
    console.error(chalk.red(`Error processing template directory: ${sourceDir}/${relativePath}`), error);
    throw error;
  }
}